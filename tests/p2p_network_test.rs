...
use aex::{
    connection::global::GlobalContext, connection::heartbeat::HeartbeatConfig,
    connection::node::Node as AexNode, crypto::session_key_manager::PairedSessionKey,
    server::HTTPServer, tcp::router::Router as TcpRouter, tcp::types::Codec,
};
...
fn create_global(addr: SocketAddr) -> Arc<GlobalContext> {
    ...
    global.heartbeat_config = HeartbeatConfig::new().with_interval(60).with_timeout(30);
    Arc::new(global)
}
...
async fn test_p2p_network_node1_to_node2() {
    ...
    let session_id = send_online_command(&mut client_socket, node2_addr, &address)
        .await
        .unwrap();
    ...
    println!(
        "Frame command entity: {:?}, action: {:?}",
        _cmd.entity, _cmd.action
    );
...
async fn test_p2p_network_node2_to_node1() {
    ...
    let session_id = send_online_command(&mut node2_socket, node2_addr, &address)
        .await
        .unwrap();
    ...
    if let Ok(frame) =
        tokio::time::timeout(Duration::from_secs(5), read_frame(&mut server_socket)).await
    {
...
async fn test_p2p_network_three_nodes() {
    ...
    send_message(&mut socket_n2, &address, "Hello from Node2 to Node1")
        .await
        .unwrap();
    println!("Node2 -> Node1: Message sent");
    tokio::time::sleep(Duration::from_millis(50)).await;

    send_message(&mut socket_n3, &address, "Hello from Node3 to Node1")
        .await
        .unwrap();
...
async fn test_p2p_encrypted_message() {
    ...
    send_message(&mut client_socket, &address, encrypted_msg)
        .await
        .unwrap();
        ...
        tokio::time::timeout(Duration::from_secs(5), read_frame(&mut server_socket)).await
    {
        if let Ok(frame) = frame {
            let cmd: P2PCommand = Codec::decode(&frame.body.data).unwrap();
            println!(
                "Server received: entity={:?}, action={:?}, data_len={}",
                cmd.entity,
                cmd.action,
                cmd.data.len()
            );
