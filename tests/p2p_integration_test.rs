...
async fn test_online_command_encoding() {
    ...
    let online_cmd = OnlineCommand {session_id:vec![1,2,3,4],node, ephemeral_public_key:[0u8;32], intranet_ips: vec![], wan_ips: vec![], seeds: None };
...
async fn test_online_command_all_fields() {
    ...
    let online_cmd = OnlineCommand {session_id:vec![1,2,3,4,5],node:node.clone(),ephemeral_public_key:[1u8;32], intranet_ips: vec![], wan_ips: vec![], seeds: None };
