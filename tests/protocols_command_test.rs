...
#[test]
    fn test_entity_and_action_enums() {
        ...
        // 覆盖所有 Action 变体
        let actions = vec![
            Action::OnLine, Action::OnLineAck, Action::OffLine, Action::Ack, Action::Update,
            Action::SendText, Action::SendBinary,
            Action::Tick, Action::Check,
            Action::Call, Action::HangUp, Action::Accept, Action::Reject,
        ];
        for a in actions {
            let cloned = a.clone();
            assert_eq!(a, cloned);
    ...
    fn test_serialization_and_codec() {
        ...
        let (decoded, _): (P2PCommand, usize) = bincode::decode_from_slice(&encoded, config).expect("Decode failed");
    ...
    fn test_p2p_command_codec_coverage() {
        ...
        // 触发 Encode 宏生成的代码
        let encoded = encode_to_vec(&cmd, config)
            .expect("Failed to encode P2PCommand");
        
        // 确保编码后的数据不为空
        assert!(!encoded.is_empty());

        // 触发 Decode 宏生成的代码
        let (decoded, len): (P2PCommand, usize) = decode_from_slice(&encoded, config)
            .expect("Failed to decode P2PCommand");
    ...
    fn test_all_variants_encode_decode() {
        ...
        for (e, a) in cases {
            ...
            let (decoded, _): (P2PCommand, usize) = bincode::decode_from_slice(&encoded, bincode::config::standard()).unwrap();
    ...
    fn test_p2p_command_encode_full_coverage() {
        ...
        let config = config::standard();
        let encoded_start = encode_to_vec(&cmd_start, config)
            .expect("Encode failed for cmd_start");
        ...
        let encoded_end = encode_to_vec(&cmd_end, config)
            .expect("Encode failed for cmd_end");
    ...
    fn test_all_entity_variants_codec() {
        ...
        for entity in all_entities {
            ...
            // 触发 Decode
            let (decoded, _): (Entity, usize) = decode_from_slice(&encoded, config).expect("Entity decode failed");
    ...
    fn test_all_action_variants_codec() {
        ...
        for action in all_actions {
            ...
            // 触发 Decode
            let (decoded, _): (Action, usize) = decode_from_slice(&encoded, config).expect("Action decode failed");
