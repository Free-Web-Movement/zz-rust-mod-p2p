...
mod tests {
    ...
    async fn test_server_list_persistence_lifecycle() {
        ...
        // 定义测试用的文件路径
        let inner_file = tmp_dir
            .path()
            .join("inner.json")
            .to_str()
            .unwrap()
            .to_string();
        let external_file = tmp_dir
            .path()
            .join("external.json")
            .to_str()
            .unwrap()
        ...
        // ==========================================
        {
            ...
            assert!(
                inner.is_empty(),
                "Initial inner server list should be empty"
            );
        ...
        // ==========================================
        {
            ...
            println!(
                "Stage 2: Successfully reloaded {} records",
                inner_reloaded.len()
            );
        }
        ...
        // ==========================================
        {
            ...
            assert!(
                external.is_empty(),
                "External server list should still be empty"
            );
        }
    }
}
